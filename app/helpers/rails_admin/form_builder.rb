    ...
    def field_wrapper_for(field, nested_in)
      if field.label
        ...
        unless nested_field_association?(field, nested_in)
          @template.content_tag(:div, class: "#{field.type} form-group control-group #{field.type_css_class} #{field.css_class} #{'error' if field.errors.present?}", id: "#{dom_id(field)}_field") do
            label(field.method_name, capitalize_first_letter(field.label), class: 'control-label') +
